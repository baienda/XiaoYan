...
//        获得默认第一个view
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (higerViewWidth <= 0 || higerViewHeight <= 0) {
            higerViewWidth = mHigherView.getMeasuredWidth();
            higerViewHeight = mHigherView.getMeasuredHeight();
        }
...
//        设置控件水平居中
        ((MarginLayoutParams) layoutParams).setMargins(-(layoutParams.width - zoomViewWidth) / 2, -(layoutParams.height - zoomViewHeight) / 2, -(layoutParams.width - zoomViewWidth) / 2, -(layoutParams.height - zoomViewHeight) / 2);
        zoomView.setLayoutParams(layoutParams);
    }


    private View mHigherView;
    private int higerViewHeight = 0;
    private int higerViewWidth = 0;

    public void setHigherView(View view) {
        this.mHigherView = view;
    }

    private void setHigher(float s) {
        float scaleTimes = (float) ((zoomViewWidth + s) / (zoomViewWidth * 1.0));
//        如超过最大放大倍数，直接返回
        if (scaleTimes > mScaleTimes) return;
        ViewGroup.LayoutParams layoutParams = mHigherView.getLayoutParams();
        layoutParams.height = (int) (higerViewHeight * scaleTimes);
        mHigherView.setLayoutParams(layoutParams);
    }
    ...
    private void replyHigherView() {
        final float distance = mHigherView.getMeasuredHeight() - higerViewHeight;
        // 设置动画
        ValueAnimator anim = ObjectAnimator.ofFloat(distance, 0.0F).setDuration((long) (distance * mReplyRatio));
        anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                setHigher((Float) animation.getAnimatedValue());
            }
        });
        anim.start();
    }
